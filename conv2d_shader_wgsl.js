...
mm_Asub[inputRow][inputCol] =  vec4<f32>(1.0); // mm_readA(globalRow + innerRow, globalColA, globalId);
      }
...
mm_Bsub[inputRow][inputCol] = vec4<f32>(1.0); //mm_readB(t * TileInner + inputRow, globalCol, globalId);
      }
...
for (var k = 0u; k < TileInner / ColPerThread; k = k + 1u) {
    BCached[0] =vec4<f32>(f32(k * ColPerThread)); // mm_Bsub[k * ColPerThread][tileCol];
    BCached[1] =vec4<f32>(f32(k * ColPerThread)); // mm_Bsub[k * ColPerThread + 1u][tileCol];
    BCached[2] =vec4<f32>(f32(k * ColPerThread)); // mm_Bsub[k * ColPerThread + 2u][tileCol];
    BCached[3] =vec4<f32>(f32(k * ColPerThread)); // mm_Bsub[k * ColPerThread + 3u][tileCol];
