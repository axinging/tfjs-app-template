export const computeShaderCode = `
    ...
    fn boolToF32(value : bool) -> f32 {
      ...
      fn boolToF32Vec4(value : vec4<bool>) -> vec4<f32> {
        return vec4<f32>(boolToF32(value[0]), boolToF32(value[1]), boolToF32(value[2]), boolToF32(value[3]));
      }
    
      fn binaryOperation(a : f32, b : f32) -> f32 {
        if (a == b) { return 1.0; } return 0.0;
      }
    ...
    [[block]] struct MatrixI32 {
        numbers: array<i32>;
      };

    [[group(0), binding(0)]] var<storage> firstMatrix : [[access(read)]] MatrixI32;
    [[group(0), binding(1)]] var<storage> secondMatrix : [[access(read)]] MatrixI32;
    ...
    fn main([[builtin(global_invocation_id)]] global_id : vec3<u32>) {
      ...
      let af: f32 = f32(firstMatrix.numbers[index]);
      let bf: f32 = f32(secondMatrix.numbers[index]);
      let cf = binaryOperation(af, bf);

      let resBool : vec4<bool> = vec4<bool>(false, true, true, false);
      var resFloat : vec4<f32> = vec4<f32>(1.0, 2.0, 3.0, 4.0);
      for (var i : u32 = 0u; i < 4u; i = i + 1u ) {
            resFloat[i] = boolToF32(resBool[i]);
      }
      resultMatrix.numbers[index] = resFloat[0] + resFloat[1] + resFloat[2];
